...
force_stop_app() {
    ...
    if [ -f "$APP_DIR/docker-compose.yaml" ]; then
        docker compose -f "$APP_DIR/docker-compose.yaml" stop 2>/dev/null || true
    else
        WARN "Compose file not found at $APP_DIR/docker-compose.yaml; relying on running-container check."
    fi
    local still_up
