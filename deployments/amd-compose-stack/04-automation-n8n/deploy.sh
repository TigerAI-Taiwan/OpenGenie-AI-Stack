...
case "$ACTION" in
    ...
    restart)
        LOG " Restarting n8n..."
        # Full redeploy (match amd): `docker compose restart` does NOT re-apply
        # env/compose changes and does NOT respect --scale (worker count), so it
        # cannot reliably restore the multi-worker queue setup. Redeploy instead.
